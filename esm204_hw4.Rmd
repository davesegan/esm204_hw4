...
library(purrr)
library(pracma) # fzero function is like solver
```
...
Present value utility with climate change:
```{r}
pv_U_with_cc
```
...
Percentage loss in present value utility from climate change:

```{r}
percent(pct_loss_L)
```

##### b. Sensitivity analysis: 10% increase in parameters
...
# New L
percent(pct_loss_L_T_new)

# % change in L
percent(pct_change_L_T_new)
...
# New L
percent(pct_loss_L_g_new)
# % change in L
percent(pct_change_L_g_new)
...
# New L
percent(pct_loss_L_n_new)

# % change in L
percent(pct_change_L_n_new)
```
...
# New L
percent(pct_loss_L_beta_new)
# % change in L
percent(pct_change_L_beta_new)
```
...
# Uncertainty parameters for T and their associat
